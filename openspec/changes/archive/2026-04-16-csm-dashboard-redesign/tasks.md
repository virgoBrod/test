...
- [x] 6.3 Visual verification: Navigate through all pages (Overview → Apps → Run test → Check history)
- [ ] 6.4 Verify project selector persists across page refresh
- [ ] 6.5 Verify credentials are saved and loaded per project

## Phase 7: Cleanup

- [x] 7.1 Remove or deprecate old `HistoryPanel.tsx` and `CollectionCard.tsx` components (not imported anywhere)
- [x] 7.2 Verify `app/globals.css`: Tailwind v4 already working correctly with new components
